...
import { createNews } from '../../../../utils/utils';
...
const NewsForm = (props) => {
    const { recordForEdit: news } = props;
    ...
    useEffect(() => {
        ...
        if (news) {
            setValues({
                ...
                newsSnippet: news.snippet,
                newsDescription: news.details,
            });
            if (news?.images) {
                news?.images.map((image) => addPrevImage(image));
            }
            if (news?.videos) {
                news?.videos.map((video) => addPrevVideo(video));
            }
        }
